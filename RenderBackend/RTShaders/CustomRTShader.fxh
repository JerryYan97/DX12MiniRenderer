#if 0
;
; Note: shader requires additional functionality:
;       Double-precision floating point
;       Typed UAV Load Additional Formats
;
; shader hash: ea8f34d01a0e3cff029005378913c898
;
; Buffer Definitions:
;
; cbuffer FrameConstantBuffer
; {
;
;   struct FrameConstantBuffer
;   {
;
;       float4 cameraPos;                             ; Offset:    0
;       float4 cameraDir;                             ; Offset:   16
;       float4 cameraUp;                              ; Offset:   32
;       float4 cameraRight;                           ; Offset:   48
;       float4 cameraInfo;                            ; Offset:   64
;       uint4 frameUintInfo;                          ; Offset:   80
;   
;   } FrameConstantBuffer;                            ; Offset:    0 Size:    96
;
; }
;
; Resource bind info for instsInfo
; {
;
;   struct struct.InstInfo
;   {
;
;       uint4 instUintInfo0;                          ; Offset:    0
;       float4 instFloatInfo0;                        ; Offset:   16
;       float4 instAlbedo;                            ; Offset:   32
;       float4 instMetallicRoughness;                 ; Offset:   48
;   
;   } $Element;                                       ; Offset:    0 Size:    64
;
; }
;
; Resource bind info for sceneVertices
; {
;
;   struct struct.VertexRaw
;   {
;
;       float4 data[3];                               ; Offset:    0
;   
;   } $Element;                                       ; Offset:    0 Size:    48
;
; }
;
;
; Resource Bindings:
;
; Name                                 Type  Format         Dim      ID      HLSL Bind  Count
; ------------------------------ ---------- ------- ----------- ------- -------------- ------
; FrameConstantBuffer               cbuffer      NA          NA     CB0            cb0     1
; scene                             texture     i32         ras      T0             t0     1
; instsInfo                         texture  struct         r/o      T1             t1     1
; sceneVertices                     texture  struct         r/o      T2             t2     1
; sceneIndices                      texture    byte         r/o      T3             t3     1
; uavRTPresent                          UAV     f32          2d      U0             u0     1
; uavRTRadiance                         UAV     f32          2d      U1             u1     1
;
target datalayout = "e-m:e-p:32:32-i1:32-i8:32-i16:32-i32:32-i64:64-f16:32-f32:32-f64:64-n8:16:32:64"
target triple = "dxil-ms-dx"

%struct.RaytracingAccelerationStructure = type { i32 }
%"class.StructuredBuffer<InstInfo>" = type { %struct.InstInfo }
%struct.InstInfo = type { <4 x i32>, <4 x float>, <4 x float>, <4 x float> }
%"class.StructuredBuffer<VertexRaw>" = type { %struct.VertexRaw }
%struct.VertexRaw = type { [3 x <4 x float>] }
%struct.ByteAddressBuffer = type { i32 }
%"class.RWTexture2D<vector<float, 4> >" = type { <4 x float> }
%FrameConstantBuffer = type { <4 x float>, <4 x float>, <4 x float>, <4 x float>, <4 x float>, <4 x i32> }
%struct.Payload = type { <3 x float>, <3 x float>, <3 x float>, <3 x float>, i32, i32, i32 }
%dx.types.Handle = type { i8* }
%dx.types.CBufRet.f32 = type { float, float, float, float }
%dx.types.CBufRet.i32 = type { i32, i32, i32, i32 }
%dx.types.ResRet.f32 = type { float, float, float, float, i32 }
%struct.BuiltInTriangleIntersectionAttributes = type { <2 x float> }
%dx.types.ResRet.i32 = type { i32, i32, i32, i32, i32 }

@"\01?scene@@3URaytracingAccelerationStructure@@A" = external constant %struct.RaytracingAccelerationStructure, align 4
@"\01?instsInfo@@3V?$StructuredBuffer@UInstInfo@@@@A" = external constant %"class.StructuredBuffer<InstInfo>", align 4
@"\01?sceneVertices@@3V?$StructuredBuffer@UVertexRaw@@@@A" = external constant %"class.StructuredBuffer<VertexRaw>", align 4
@"\01?sceneIndices@@3UByteAddressBuffer@@A" = external constant %struct.ByteAddressBuffer, align 4
@"\01?uavRTPresent@@3V?$RWTexture2D@V?$vector@M$03@@@@A" = external constant %"class.RWTexture2D<vector<float, 4> >", align 4
@"\01?uavRTRadiance@@3V?$RWTexture2D@V?$vector@M$03@@@@A" = external constant %"class.RWTexture2D<vector<float, 4> >", align 4
@FrameConstantBuffer = external constant %FrameConstantBuffer

; Function Attrs: nounwind
define void @"\01?RayGeneration@@YAXXZ"() #0 {
  %1 = load %struct.RaytracingAccelerationStructure, %struct.RaytracingAccelerationStructure* @"\01?scene@@3URaytracingAccelerationStructure@@A", align 4
  %2 = load %"class.RWTexture2D<vector<float, 4> >", %"class.RWTexture2D<vector<float, 4> >"* @"\01?uavRTRadiance@@3V?$RWTexture2D@V?$vector@M$03@@@@A", align 4
  %3 = load %"class.RWTexture2D<vector<float, 4> >", %"class.RWTexture2D<vector<float, 4> >"* @"\01?uavRTPresent@@3V?$RWTexture2D@V?$vector@M$03@@@@A", align 4
  %4 = load %FrameConstantBuffer, %FrameConstantBuffer* @FrameConstantBuffer, align 4
  %5 = alloca %struct.Payload, align 4
  %6 = call %dx.types.Handle @dx.op.createHandleForLib.FrameConstantBuffer(i32 160, %FrameConstantBuffer %4)  ; CreateHandleForLib(Resource)
  %7 = call i32 @dx.op.dispatchRaysIndex.i32(i32 145, i8 0)  ; DispatchRaysIndex(col)
  %8 = call i32 @dx.op.dispatchRaysIndex.i32(i32 145, i8 1)  ; DispatchRaysIndex(col)
  %9 = call i32 @dx.op.dispatchRaysDimensions.i32(i32 146, i8 0)  ; DispatchRaysDimensions(col)
  %10 = call i32 @dx.op.dispatchRaysDimensions.i32(i32 146, i8 1)  ; DispatchRaysDimensions(col)
  %11 = uitofp i32 %9 to float
  %12 = uitofp i32 %10 to float
  %13 = uitofp i32 %7 to float
  %14 = uitofp i32 %8 to float
  %15 = fdiv fast float %13, %11
  %16 = fdiv fast float %14, %12
  %17 = call %dx.types.CBufRet.f32 @dx.op.cbufferLoadLegacy.f32(i32 59, %dx.types.Handle %6, i32 4)  ; CBufferLoadLegacy(handle,regIndex)
  %18 = extractvalue %dx.types.CBufRet.f32 %17, 0
  %19 = extractvalue %dx.types.CBufRet.f32 %17, 1
  %20 = fmul fast float %18, 5.000000e-01
  %21 = call float @dx.op.unary.f32(i32 14, float %20)  ; Tan(value)
  %22 = fmul fast float %21, %19
  %23 = fdiv fast float %11, %12
  %24 = fmul fast float %22, %23
  %25 = call %dx.types.CBufRet.f32 @dx.op.cbufferLoadLegacy.f32(i32 59, %dx.types.Handle %6, i32 0)  ; CBufferLoadLegacy(handle,regIndex)
  %26 = extractvalue %dx.types.CBufRet.f32 %25, 0
  %27 = extractvalue %dx.types.CBufRet.f32 %25, 1
  %28 = extractvalue %dx.types.CBufRet.f32 %25, 2
  %29 = call %dx.types.CBufRet.f32 @dx.op.cbufferLoadLegacy.f32(i32 59, %dx.types.Handle %6, i32 1)  ; CBufferLoadLegacy(handle,regIndex)
  %30 = extractvalue %dx.types.CBufRet.f32 %29, 0
  %31 = extractvalue %dx.types.CBufRet.f32 %29, 1
  %32 = extractvalue %dx.types.CBufRet.f32 %29, 2
  %33 = fmul fast float %30, %19
  %34 = fmul fast float %31, %19
  %35 = fmul fast float %32, %19
  %36 = fadd fast float %33, %26
  %37 = fadd fast float %34, %27
  %38 = fadd fast float %35, %28
  %39 = getelementptr inbounds %struct.Payload, %struct.Payload* %5, i32 0, i32 1
  br label %40

; <label>:40                                      ; preds = %128, %0
  %41 = phi float [ 0.000000e+00, %0 ], [ %133, %128 ]
  %42 = phi float [ 0.000000e+00, %0 ], [ %134, %128 ]
  %43 = phi float [ 0.000000e+00, %0 ], [ %135, %128 ]
  %44 = phi i32 [ 0, %0 ], [ %136, %128 ]
  %45 = uitofp i32 %44 to float
  %46 = fmul fast float %45, 7.812500e-03
  %47 = call %dx.types.CBufRet.i32 @dx.op.cbufferLoadLegacy.i32(i32 59, %dx.types.Handle %6, i32 5)  ; CBufferLoadLegacy(handle,regIndex)
  %48 = extractvalue %dx.types.CBufRet.i32 %47, 0
  %49 = uitofp i32 %48 to float
  %50 = fmul fast float %49, -6.521250e+02
  %51 = call float @dx.op.unary.f32(i32 13, float %50)  ; Sin(value)
  %52 = fmul fast float %51, 0x40934BF580000000
  %53 = call float @dx.op.unary.f32(i32 22, float %52)  ; Frc(value)
  %54 = fadd fast float %53, %46
  %55 = extractvalue %dx.types.CBufRet.i32 %47, 1
  %56 = uitofp i32 %55 to float
  %57 = fmul fast float %56, -6.521250e+02
  %58 = call float @dx.op.unary.f32(i32 13, float %57)  ; Sin(value)
  %59 = fmul fast float %58, 0x40934BF580000000
  %60 = call float @dx.op.unary.f32(i32 22, float %59)  ; Frc(value)
  %61 = fadd fast float %54, %60
  %62 = fmul fast float %61, %15
  %63 = fmul fast float %61, %16
  %64 = call float @dx.op.dot2.f32(i32 54, float %62, float %63, float 0x4039FAC720000000, float 0x40638EE980000000)  ; Dot2(ax,ay,bx,by)
  %65 = call float @dx.op.unary.f32(i32 13, float %64)  ; Sin(value)
  %66 = fmul fast float %65, 0x40E55DD180000000
  %67 = call float @dx.op.unary.f32(i32 22, float %66)  ; Frc(value)
  %68 = fmul fast float %67, %67
  %69 = fsub fast float 1.000000e+00, %68
  %70 = call float @dx.op.unary.f32(i32 24, float %69)  ; Sqrt(value)
  %71 = fadd fast float %13, -5.000000e-01
  %72 = fadd fast float %71, %67
  %73 = fdiv fast float %72, %11
  %74 = fadd fast float %14, -5.000000e-01
  %75 = fadd fast float %74, %70
  %76 = fdiv fast float %75, %12
  %77 = fmul fast float %73, 2.000000e+00
  %78 = fadd fast float %77, -1.000000e+00
  %79 = fmul fast float %24, %78
  %80 = fmul fast float %76, 2.000000e+00
  %81 = fsub fast float 1.000000e+00, %80
  %82 = fmul fast float %81, %22
  %83 = fadd fast float %36, %79
  %84 = call %dx.types.CBufRet.f32 @dx.op.cbufferLoadLegacy.f32(i32 59, %dx.types.Handle %6, i32 0)  ; CBufferLoadLegacy(handle,regIndex)
  %85 = extractvalue %dx.types.CBufRet.f32 %84, 0
  %86 = extractvalue %dx.types.CBufRet.f32 %84, 1
  %87 = extractvalue %dx.types.CBufRet.f32 %84, 2
  %88 = insertelement <3 x float> undef, float %85, i32 0
  %89 = insertelement <3 x float> %88, float %86, i32 1
  %90 = insertelement <3 x float> %89, float %87, i32 2
  %91 = fsub fast float %83, %85
  %92 = fsub fast float %37, %86
  %93 = fadd fast float %92, %82
  %94 = fsub fast float %38, %87
  %95 = insertelement <3 x float> undef, float %91, i32 0
  %96 = insertelement <3 x float> %95, float %93, i32 1
  %97 = insertelement <3 x float> %96, float %94, i32 2
  %98 = getelementptr inbounds %struct.Payload, %struct.Payload* %5, i32 0, i32 5
  store i32 0, i32* %98, align 4, !tbaa !36
  %99 = getelementptr inbounds %struct.Payload, %struct.Payload* %5, i32 0, i32 6
  store i32 0, i32* %99, align 4, !tbaa !36
  store <3 x float> zeroinitializer, <3 x float>* %39, align 4, !tbaa !40
  %100 = getelementptr inbounds %struct.Payload, %struct.Payload* %5, i32 0, i32 0
  store <3 x float> <float 1.000000e+00, float 1.000000e+00, float 1.000000e+00>, <3 x float>* %100, align 4, !tbaa !40
  %101 = getelementptr inbounds %struct.Payload, %struct.Payload* %5, i32 0, i32 3
  store <3 x float> zeroinitializer, <3 x float>* %101, align 4, !tbaa !40
  %102 = getelementptr inbounds %struct.Payload, %struct.Payload* %5, i32 0, i32 2
  store <3 x float> zeroinitializer, <3 x float>* %102, align 4, !tbaa !40
  %103 = getelementptr inbounds %struct.Payload, %struct.Payload* %5, i32 0, i32 4
  store i32 0, i32* %103, align 4, !tbaa !41
  br label %104

; <label>:104                                     ; preds = %117, %40
  %105 = phi i32 [ %127, %117 ], [ 0, %40 ]
  %106 = phi <3 x float> [ %126, %117 ], [ %97, %40 ]
  %107 = phi <3 x float> [ %125, %117 ], [ %90, %40 ]
  %108 = icmp eq i32 %105, 0
  %109 = load i32, i32* %103, align 4
  %110 = icmp ult i32 %109, 5
  %111 = and i1 %108, %110
  br i1 %111, label %112, label %115

; <label>:112                                     ; preds = %104
  %113 = load i32, i32* %98, align 4, !tbaa !36, !range !43
  %114 = icmp eq i32 %113, 0
  br label %115

; <label>:115                                     ; preds = %112, %104
  %116 = phi i1 [ false, %104 ], [ %114, %112 ]
  br i1 %116, label %117, label %128

; <label>:117                                     ; preds = %115
  %118 = call %dx.types.Handle @dx.op.createHandleForLib.struct.RaytracingAccelerationStructure(i32 160, %struct.RaytracingAccelerationStructure %1)  ; CreateHandleForLib(Resource)
  %119 = extractelement <3 x float> %107, i64 0
  %120 = extractelement <3 x float> %107, i64 1
  %121 = extractelement <3 x float> %107, i64 2
  %122 = extractelement <3 x float> %106, i64 0
  %123 = extractelement <3 x float> %106, i64 1
  %124 = extractelement <3 x float> %106, i64 2
  call void @dx.op.traceRay.struct.Payload(i32 157, %dx.types.Handle %118, i32 0, i32 255, i32 0, i32 0, i32 0, float %119, float %120, float %121, float 0x3F1A36E2E0000000, float %122, float %123, float %124, float 1.000000e+03, %struct.Payload* nonnull %5)  ; TraceRay(AccelerationStructure,RayFlags,InstanceInclusionMask,RayContributionToHitGroupIndex,MultiplierForGeometryContributionToShaderIndex,MissShaderIndex,Origin_X,Origin_Y,Origin_Z,TMin,Direction_X,Direction_Y,Direction_Z,TMax,payload)
  %125 = load <3 x float>, <3 x float>* %102, align 4, !tbaa !40
  %126 = load <3 x float>, <3 x float>* %101, align 4, !tbaa !40
  %127 = load i32, i32* %99, align 4, !tbaa !36
  br label %104

; <label>:128                                     ; preds = %115
  %129 = load <3 x float>, <3 x float>* %39, align 4, !tbaa !40
  %130 = extractelement <3 x float> %129, i64 0
  %131 = extractelement <3 x float> %129, i64 1
  %132 = extractelement <3 x float> %129, i64 2
  %133 = fadd fast float %130, %41
  %134 = fadd fast float %131, %42
  %135 = fadd fast float %132, %43
  %136 = add nuw nsw i32 %44, 1
  %137 = icmp eq i32 %136, 128
  br i1 %137, label %138, label %40

; <label>:138                                     ; preds = %128
  %139 = fmul fast float %133, 7.812500e-03
  %140 = fmul fast float %134, 7.812500e-03
  %141 = fmul fast float %135, 7.812500e-03
  %142 = call %dx.types.Handle @"dx.op.createHandleForLib.class.RWTexture2D<vector<float, 4> >"(i32 160, %"class.RWTexture2D<vector<float, 4> >" %2)  ; CreateHandleForLib(Resource)
  %143 = call %dx.types.ResRet.f32 @dx.op.textureLoad.f32(i32 66, %dx.types.Handle %142, i32 undef, i32 %7, i32 %8, i32 undef, i32 undef, i32 undef, i32 undef)  ; TextureLoad(srv,mipLevelOrSampleCount,coord0,coord1,coord2,offset0,offset1,offset2)
  %144 = extractvalue %dx.types.ResRet.f32 %143, 0
  %145 = extractvalue %dx.types.ResRet.f32 %143, 1
  %146 = extractvalue %dx.types.ResRet.f32 %143, 2
  %147 = extractvalue %dx.types.ResRet.f32 %143, 3
  %148 = call %dx.types.CBufRet.i32 @dx.op.cbufferLoadLegacy.i32(i32 59, %dx.types.Handle %6, i32 5)  ; CBufferLoadLegacy(handle,regIndex)
  %149 = extractvalue %dx.types.CBufRet.i32 %148, 0
  %150 = uitofp i32 %149 to float
  %151 = fmul fast float %150, %144
  %152 = fmul fast float %150, %145
  %153 = fmul fast float %150, %146
  %154 = fmul fast float %150, %147
  %155 = fadd fast float %151, %139
  %156 = fadd fast float %152, %140
  %157 = fadd fast float %153, %141
  %158 = fadd fast float %154, 1.000000e+00
  %159 = add i32 %149, 1
  %160 = uitofp i32 %159 to float
  %161 = fdiv fast float %155, %160
  %162 = fdiv fast float %156, %160
  %163 = fdiv fast float %157, %160
  %164 = fdiv fast float %158, %160
  call void @dx.op.textureStore.f32(i32 67, %dx.types.Handle %142, i32 %7, i32 %8, i32 undef, float %161, float %162, float %163, float %164, i8 15)  ; TextureStore(srv,coord0,coord1,coord2,value0,value1,value2,value3,mask)
  %165 = fadd fast float %161, 1.000000e+00
  %166 = fadd fast float %162, 1.000000e+00
  %167 = fadd fast float %163, 1.000000e+00
  %168 = fdiv fast float %161, %165
  %169 = fdiv fast float %162, %166
  %170 = fdiv fast float %163, %167
  %171 = call float @dx.op.unary.f32(i32 23, float %168)  ; Log(value)
  %172 = call float @dx.op.unary.f32(i32 23, float %169)  ; Log(value)
  %173 = call float @dx.op.unary.f32(i32 23, float %170)  ; Log(value)
  %174 = fmul fast float %171, 0x3FDD1745E0000000
  %175 = fmul fast float %172, 0x3FDD1745E0000000
  %176 = fmul fast float %173, 0x3FDD1745E0000000
  %177 = call float @dx.op.unary.f32(i32 21, float %174)  ; Exp(value)
  %178 = call float @dx.op.unary.f32(i32 21, float %175)  ; Exp(value)
  %179 = call float @dx.op.unary.f32(i32 21, float %176)  ; Exp(value)
  %180 = call %dx.types.Handle @"dx.op.createHandleForLib.class.RWTexture2D<vector<float, 4> >"(i32 160, %"class.RWTexture2D<vector<float, 4> >" %3)  ; CreateHandleForLib(Resource)
  call void @dx.op.textureStore.f32(i32 67, %dx.types.Handle %180, i32 %7, i32 %8, i32 undef, float %177, float %178, float %179, float 1.000000e+00, i8 15)  ; TextureStore(srv,coord0,coord1,coord2,value0,value1,value2,value3,mask)
  ret void
}

; Function Attrs: nounwind
define void @"\01?Miss@@YAXUPayload@@@Z"(%struct.Payload* noalias nocapture %payload) #0 {
  %1 = load %FrameConstantBuffer, %FrameConstantBuffer* @FrameConstantBuffer, align 4
  %2 = call %dx.types.Handle @dx.op.createHandleForLib.FrameConstantBuffer(i32 160, %FrameConstantBuffer %1)  ; CreateHandleForLib(Resource)
  %3 = call %dx.types.CBufRet.i32 @dx.op.cbufferLoadLegacy.i32(i32 59, %dx.types.Handle %2, i32 5)  ; CBufferLoadLegacy(handle,regIndex)
  %4 = extractvalue %dx.types.CBufRet.i32 %3, 3
  %5 = icmp eq i32 %4, 1
  br i1 %5, label %6, label %34

; <label>:6                                       ; preds = %0
  %7 = call float @dx.op.worldRayDirection.f32(i32 148, i8 0)  ; WorldRayDirection(col)
  %8 = call float @dx.op.worldRayDirection.f32(i32 148, i8 1)  ; WorldRayDirection(col)
  %9 = call float @dx.op.worldRayDirection.f32(i32 148, i8 2)  ; WorldRayDirection(col)
  %10 = call float @dx.op.dot3.f32(i32 55, float %7, float %8, float %9, float %7, float %8, float %9)  ; Dot3(ax,ay,az,bx,by,bz)
  %11 = call float @dx.op.unary.f32(i32 25, float %10)  ; Rsqrt(value)
  %12 = fmul fast float %8, 5.000000e+00
  %13 = fmul fast float %12, %11
  %14 = fadd fast float %13, 5.000000e-01
  %15 = call float @dx.op.unary.f32(i32 7, float %14)  ; Saturate(value)
  %16 = getelementptr inbounds %struct.Payload, %struct.Payload* %payload, i32 0, i32 0
  %17 = load <3 x float>, <3 x float>* %16, align 4, !tbaa !40
  %18 = extractelement <3 x float> %17, i32 0
  %19 = extractelement <3 x float> %17, i32 1
  %20 = extractelement <3 x float> %17, i32 2
  %21 = fmul fast float %15, 0x3FE051EB80000000
  %22 = fmul fast float %15, 0x3FDAE147C0000000
  %23 = fmul fast float %15, 0x3FCAE14780000000
  %24 = fsub fast float 7.500000e-01, %21
  %25 = fsub fast float 0x3FEB851EC0000000, %22
  %26 = fsub fast float 0x3FEDC28F60000000, %23
  %27 = fmul fast float %18, %24
  %28 = fmul fast float %19, %25
  %29 = fmul fast float %20, %26
  %30 = insertelement <3 x float> undef, float %27, i32 0
  %31 = insertelement <3 x float> %30, float %28, i32 1
  %32 = insertelement <3 x float> %31, float %29, i32 2
  %33 = getelementptr inbounds %struct.Payload, %struct.Payload* %payload, i32 0, i32 1
  store <3 x float> %32, <3 x float>* %33, align 4, !tbaa !40
  br label %34

; <label>:34                                      ; preds = %6, %0
  %35 = getelementptr inbounds %struct.Payload, %struct.Payload* %payload, i32 0, i32 6
  store i32 1, i32* %35, align 4, !tbaa !36
  ret void
}

; Function Attrs: nounwind
define void @"\01?ClosestHit@@YAXUPayload@@UBuiltInTriangleIntersectionAttributes@@@Z"(%struct.Payload* noalias nocapture %payload, %struct.BuiltInTriangleIntersectionAttributes* nocapture readonly %attrib) #0 {
  %1 = load %struct.ByteAddressBuffer, %struct.ByteAddressBuffer* @"\01?sceneIndices@@3UByteAddressBuffer@@A", align 4
  %2 = load %"class.StructuredBuffer<VertexRaw>", %"class.StructuredBuffer<VertexRaw>"* @"\01?sceneVertices@@3V?$StructuredBuffer@UVertexRaw@@@@A", align 4, !noalias !44
  %3 = load %"class.StructuredBuffer<InstInfo>", %"class.StructuredBuffer<InstInfo>"* @"\01?instsInfo@@3V?$StructuredBuffer@UInstInfo@@@@A", align 4
  %4 = load %FrameConstantBuffer, %FrameConstantBuffer* @FrameConstantBuffer, align 4
  %5 = call %dx.types.Handle @dx.op.createHandleForLib.FrameConstantBuffer(i32 160, %FrameConstantBuffer %4)  ; CreateHandleForLib(Resource)
  %6 = call i32 @dx.op.instanceID.i32(i32 141)  ; InstanceID()
  %7 = call %dx.types.Handle @"dx.op.createHandleForLib.class.StructuredBuffer<InstInfo>"(i32 160, %"class.StructuredBuffer<InstInfo>" %3)  ; CreateHandleForLib(Resource)
  %8 = call %dx.types.ResRet.i32 @dx.op.rawBufferLoad.i32(i32 139, %dx.types.Handle %7, i32 %6, i32 0, i8 15, i32 4)  ; RawBufferLoad(srv,index,elementOffset,mask,alignment)
  %9 = extractvalue %dx.types.ResRet.i32 %8, 0
  %10 = call %dx.types.ResRet.f32 @dx.op.rawBufferLoad.f32(i32 139, %dx.types.Handle %7, i32 %6, i32 16, i8 15, i32 4)  ; RawBufferLoad(srv,index,elementOffset,mask,alignment)
  %11 = extractvalue %dx.types.ResRet.f32 %10, 0
  %12 = extractvalue %dx.types.ResRet.f32 %10, 1
  %13 = extractvalue %dx.types.ResRet.f32 %10, 2
  %14 = call %dx.types.ResRet.f32 @dx.op.rawBufferLoad.f32(i32 139, %dx.types.Handle %7, i32 %6, i32 32, i8 15, i32 4)  ; RawBufferLoad(srv,index,elementOffset,mask,alignment)
  %15 = extractvalue %dx.types.ResRet.f32 %14, 0
  %16 = extractvalue %dx.types.ResRet.f32 %14, 1
  %17 = extractvalue %dx.types.ResRet.f32 %14, 2
  %18 = call %dx.types.ResRet.f32 @dx.op.rawBufferLoad.f32(i32 139, %dx.types.Handle %7, i32 %6, i32 48, i8 15, i32 4)  ; RawBufferLoad(srv,index,elementOffset,mask,alignment)
  %19 = extractvalue %dx.types.ResRet.f32 %18, 0
  %20 = extractvalue %dx.types.ResRet.f32 %18, 1
  %21 = getelementptr inbounds %struct.Payload, %struct.Payload* %payload, i32 0, i32 4
  %22 = load i32, i32* %21, align 4, !tbaa !41
  %23 = add i32 %22, 1
  store i32 %23, i32* %21, align 4, !tbaa !41
  %24 = fmul fast float %11, %11
  %25 = fmul fast float %12, %12
  %26 = fadd fast float %24, %25
  %27 = fmul fast float %13, %13
  %28 = fadd fast float %26, %27
  %29 = call float @dx.op.unary.f32(i32 24, float %28)  ; Sqrt(value)
  %30 = fcmp fast ogt float %29, 0.000000e+00
  br i1 %30, label %31, label %45

; <label>:31                                      ; preds = %0
  %32 = getelementptr inbounds %struct.Payload, %struct.Payload* %payload, i32 0, i32 5
  store i32 1, i32* %32, align 4, !tbaa !36
  %33 = getelementptr inbounds %struct.Payload, %struct.Payload* %payload, i32 0, i32 0
  %34 = load <3 x float>, <3 x float>* %33, align 4, !tbaa !40
  %35 = extractelement <3 x float> %34, i32 0
  %36 = fmul fast float %35, %11
  %37 = extractelement <3 x float> %34, i32 1
  %38 = fmul fast float %37, %12
  %39 = extractelement <3 x float> %34, i32 2
  %40 = fmul fast float %39, %13
  %41 = insertelement <3 x float> undef, float %36, i32 0
  %42 = insertelement <3 x float> %41, float %38, i32 1
  %43 = insertelement <3 x float> %42, float %40, i32 2
  %44 = getelementptr inbounds %struct.Payload, %struct.Payload* %payload, i32 0, i32 1
  store <3 x float> %43, <3 x float>* %44, align 4, !tbaa !40
  br label %472

; <label>:45                                      ; preds = %0
  %46 = extractvalue %dx.types.ResRet.i32 %8, 1
  %47 = extractvalue %dx.types.ResRet.i32 %8, 2
  %48 = shl i32 %47, 1
  %49 = call i32 @dx.op.primitiveIndex.i32(i32 161)  ; PrimitiveIndex()
  %50 = mul i32 %49, 6
  %51 = add i32 %50, %48
  %52 = and i32 %51, -4
  %53 = call %dx.types.Handle @dx.op.createHandleForLib.struct.ByteAddressBuffer(i32 160, %struct.ByteAddressBuffer %1)  ; CreateHandleForLib(Resource)
  %54 = call %dx.types.ResRet.i32 @dx.op.rawBufferLoad.i32(i32 139, %dx.types.Handle %53, i32 %52, i32 undef, i8 3, i32 4)  ; RawBufferLoad(srv,index,elementOffset,mask,alignment)
  %55 = extractvalue %dx.types.ResRet.i32 %54, 0
  %56 = extractvalue %dx.types.ResRet.i32 %54, 1
  %57 = icmp eq i32 %52, %51
  %58 = and i32 %55, 65535
  %59 = lshr i32 %55, 16
  %60 = lshr i32 %56, 16
  %61 = select i1 %57, i32 %56, i32 %60
  %62 = select i1 %57, i32 %58, i32 %59
  %63 = select i1 %57, i32 %59, i32 %56
  %64 = and i32 %63, 65535
  %65 = and i32 %61, 65535
  %66 = udiv i32 %46, 12
  %67 = add nuw nsw i32 %62, %66
  %68 = call %dx.types.Handle @"dx.op.createHandleForLib.class.StructuredBuffer<VertexRaw>"(i32 160, %"class.StructuredBuffer<VertexRaw>" %2)  ; CreateHandleForLib(Resource)
  %69 = call %dx.types.ResRet.f32 @dx.op.rawBufferLoad.f32(i32 139, %dx.types.Handle %68, i32 %67, i32 0, i8 15, i32 4)  ; RawBufferLoad(srv,index,elementOffset,mask,alignment)
  %70 = extractvalue %dx.types.ResRet.f32 %69, 3
  %71 = call %dx.types.ResRet.f32 @dx.op.rawBufferLoad.f32(i32 139, %dx.types.Handle %68, i32 %67, i32 16, i8 15, i32 4)  ; RawBufferLoad(srv,index,elementOffset,mask,alignment)
  %72 = extractvalue %dx.types.ResRet.f32 %71, 0
  %73 = extractvalue %dx.types.ResRet.f32 %71, 1
  %74 = add nuw nsw i32 %64, %66
  %75 = call %dx.types.ResRet.f32 @dx.op.rawBufferLoad.f32(i32 139, %dx.types.Handle %68, i32 %74, i32 0, i8 15, i32 4)  ; RawBufferLoad(srv,index,elementOffset,mask,alignment)
  %76 = extractvalue %dx.types.ResRet.f32 %75, 3
  %77 = call %dx.types.ResRet.f32 @dx.op.rawBufferLoad.f32(i32 139, %dx.types.Handle %68, i32 %74, i32 16, i8 15, i32 4)  ; RawBufferLoad(srv,index,elementOffset,mask,alignment)
  %78 = extractvalue %dx.types.ResRet.f32 %77, 0
  %79 = extractvalue %dx.types.ResRet.f32 %77, 1
  %80 = add nuw nsw i32 %65, %66
  %81 = call %dx.types.ResRet.f32 @dx.op.rawBufferLoad.f32(i32 139, %dx.types.Handle %68, i32 %80, i32 0, i8 15, i32 4)  ; RawBufferLoad(srv,index,elementOffset,mask,alignment)
  %82 = extractvalue %dx.types.ResRet.f32 %81, 3
  %83 = call %dx.types.ResRet.f32 @dx.op.rawBufferLoad.f32(i32 139, %dx.types.Handle %68, i32 %80, i32 16, i8 15, i32 4)  ; RawBufferLoad(srv,index,elementOffset,mask,alignment)
  %84 = extractvalue %dx.types.ResRet.f32 %83, 0
  %85 = extractvalue %dx.types.ResRet.f32 %83, 1
  %86 = getelementptr inbounds %struct.BuiltInTriangleIntersectionAttributes, %struct.BuiltInTriangleIntersectionAttributes* %attrib, i32 0, i32 0
  %87 = load <2 x float>, <2 x float>* %86, align 4
  %88 = extractelement <2 x float> %87, i32 0
  %89 = fsub fast float %76, %70
  %90 = fsub fast float %78, %72
  %91 = fsub fast float %79, %73
  %92 = fmul fast float %88, %89
  %93 = fmul fast float %88, %90
  %94 = fmul fast float %88, %91
  %95 = fadd fast float %92, %70
  %96 = fadd fast float %93, %72
  %97 = fadd fast float %94, %73
  %98 = extractelement <2 x float> %87, i32 1
  %99 = fsub fast float %82, %70
  %100 = fsub fast float %84, %72
  %101 = fsub fast float %85, %73
  %102 = fmul fast float %98, %99
  %103 = fmul fast float %98, %100
  %104 = fmul fast float %98, %101
  %105 = fadd fast float %95, %102
  %106 = fadd fast float %96, %103
  %107 = fadd fast float %97, %104
  %108 = call float @dx.op.worldRayOrigin.f32(i32 147, i8 0)  ; WorldRayOrigin(col)
  %109 = call float @dx.op.worldRayOrigin.f32(i32 147, i8 1)  ; WorldRayOrigin(col)
  %110 = call float @dx.op.worldRayOrigin.f32(i32 147, i8 2)  ; WorldRayOrigin(col)
  %111 = call float @dx.op.worldRayDirection.f32(i32 148, i8 0)  ; WorldRayDirection(col)
  %112 = call float @dx.op.worldRayDirection.f32(i32 148, i8 1)  ; WorldRayDirection(col)
  %113 = call float @dx.op.worldRayDirection.f32(i32 148, i8 2)  ; WorldRayDirection(col)
  %114 = call float @dx.op.rayTCurrent.f32(i32 154)  ; RayTCurrent()
  %115 = fmul fast float %114, %111
  %116 = fmul fast float %114, %112
  %117 = fmul fast float %114, %113
  %118 = fadd fast float %115, %108
  %119 = fadd fast float %116, %109
  %120 = fadd fast float %117, %110
  %121 = insertelement <3 x float> undef, float %118, i32 0
  %122 = insertelement <3 x float> %121, float %119, i32 1
  %123 = insertelement <3 x float> %122, float %120, i32 2
  %124 = call float @dx.op.objectToWorld.f32(i32 151, i32 0, i8 0)  ; ObjectToWorld(row,col)
  %125 = call float @dx.op.objectToWorld.f32(i32 151, i32 0, i8 1)  ; ObjectToWorld(row,col)
  %126 = call float @dx.op.objectToWorld.f32(i32 151, i32 0, i8 2)  ; ObjectToWorld(row,col)
  %127 = call float @dx.op.objectToWorld.f32(i32 151, i32 1, i8 0)  ; ObjectToWorld(row,col)
  %128 = call float @dx.op.objectToWorld.f32(i32 151, i32 1, i8 1)  ; ObjectToWorld(row,col)
  %129 = call float @dx.op.objectToWorld.f32(i32 151, i32 1, i8 2)  ; ObjectToWorld(row,col)
  %130 = call float @dx.op.objectToWorld.f32(i32 151, i32 2, i8 0)  ; ObjectToWorld(row,col)
  %131 = call float @dx.op.objectToWorld.f32(i32 151, i32 2, i8 1)  ; ObjectToWorld(row,col)
  %132 = call float @dx.op.objectToWorld.f32(i32 151, i32 2, i8 2)  ; ObjectToWorld(row,col)
  %133 = call float @dx.op.dot3.f32(i32 55, float %105, float %106, float %107, float %105, float %106, float %107)  ; Dot3(ax,ay,az,bx,by,bz)
  %134 = call float @dx.op.unary.f32(i32 25, float %133)  ; Rsqrt(value)
  %135 = fmul fast float %134, %105
  %136 = fmul fast float %134, %106
  %137 = fmul fast float %134, %107
  %138 = fmul fast float %135, %124
  %139 = call float @dx.op.tertiary.f32(i32 46, float %125, float %136, float %138)  ; FMad(a,b,c)
  %140 = call float @dx.op.tertiary.f32(i32 46, float %126, float %137, float %139)  ; FMad(a,b,c)
  %141 = fmul fast float %135, %127
  %142 = call float @dx.op.tertiary.f32(i32 46, float %128, float %136, float %141)  ; FMad(a,b,c)
  %143 = call float @dx.op.tertiary.f32(i32 46, float %129, float %137, float %142)  ; FMad(a,b,c)
  %144 = fmul fast float %135, %130
  %145 = call float @dx.op.tertiary.f32(i32 46, float %131, float %136, float %144)  ; FMad(a,b,c)
  %146 = call float @dx.op.tertiary.f32(i32 46, float %132, float %137, float %145)  ; FMad(a,b,c)
  %147 = call float @dx.op.dot3.f32(i32 55, float %140, float %143, float %146, float %140, float %143, float %146)  ; Dot3(ax,ay,az,bx,by,bz)
  %148 = call float @dx.op.unary.f32(i32 25, float %147)  ; Rsqrt(value)
  %149 = fmul fast float %148, %140
  %150 = fmul fast float %148, %143
  %151 = fmul fast float %148, %146
  %152 = fmul fast float %111, %111
  %153 = fmul fast float %112, %112
  %154 = fadd fast float %153, %152
  %155 = fmul fast float %113, %113
  %156 = fadd fast float %154, %155
  %157 = call float @dx.op.unary.f32(i32 24, float %156)  ; Sqrt(value)
  %158 = fcmp fast oeq float %157, 0.000000e+00
  %159 = select i1 %158, float %149, float %111
  %160 = select i1 %158, float %150, float %112
  %161 = select i1 %158, float %151, float %113
  %162 = call float @dx.op.dot3.f32(i32 55, float %159, float %160, float %161, float %159, float %160, float %161)  ; Dot3(ax,ay,az,bx,by,bz)
  %163 = call float @dx.op.unary.f32(i32 25, float %162)  ; Rsqrt(value)
  %164 = fmul fast float %159, %163
  %165 = fmul fast float %160, %163
  %166 = fmul fast float %161, %163
  %167 = call float @dx.op.dot3.f32(i32 55, float %164, float %165, float %166, float %149, float %150, float %151)  ; Dot3(ax,ay,az,bx,by,bz)
  %168 = and i32 %9, 64
  %169 = icmp ne i32 %168, 0
  %170 = fcmp fast oge float %167, 0.000000e+00
  %171 = and i1 %169, %170
  %172 = fsub fast float -0.000000e+00, %149
  %173 = fsub fast float -0.000000e+00, %150
  %174 = fsub fast float -0.000000e+00, %151
  %175 = select i1 %171, float %172, float %149
  %176 = select i1 %171, float %173, float %150
  %177 = select i1 %171, float %174, float %151
  %178 = and i32 %9, 31
  %179 = icmp eq i32 %178, 0
  br i1 %179, label %180, label %465

; <label>:180                                     ; preds = %45
  %181 = and i32 %9, 32
  %182 = icmp eq i32 %181, 0
  br i1 %182, label %308, label %183

; <label>:183                                     ; preds = %180
  %184 = select i1 %170, float 1.500000e+00, float 0x3FE5555560000000
  %185 = fsub fast float -0.000000e+00, %175
  %186 = fsub fast float -0.000000e+00, %176
  %187 = fsub fast float -0.000000e+00, %177
  %188 = select i1 %170, float %185, float %175
  %189 = select i1 %170, float %186, float %176
  %190 = select i1 %170, float %187, float %177
  %191 = fsub fast float -0.000000e+00, %164
  %192 = fsub fast float -0.000000e+00, %165
  %193 = fsub fast float -0.000000e+00, %166
  %194 = call float @dx.op.dot3.f32(i32 55, float %191, float %192, float %193, float %188, float %189, float %190)  ; Dot3(ax,ay,az,bx,by,bz)
  %195 = call float @dx.op.binary.f32(i32 36, float %194, float 1.000000e+00)  ; FMin(a,b)
  %196 = fmul fast float %195, %195
  %197 = fsub fast float 1.000000e+00, %196
  %198 = call float @dx.op.unary.f32(i32 24, float %197)  ; Sqrt(value)
  %199 = fpext float %198 to double
  %200 = fpext float %184 to double
  %201 = fmul fast double %199, %200
  %202 = fcmp fast ogt double %201, 1.000000e+00
  br i1 %202, label %247, label %203

; <label>:203                                     ; preds = %183
  %204 = fsub fast float 1.000000e+00, %184
  %205 = fadd fast float %184, 1.000000e+00
  %206 = fdiv fast float %204, %205
  %207 = fmul fast float %206, %206
  %208 = fsub fast float 1.000000e+00, %207
  %209 = fsub fast float 1.000000e+00, %195
  %210 = call float @dx.op.unary.f32(i32 23, float %209)  ; Log(value)
  %211 = fmul fast float %210, 5.000000e+00
  %212 = call float @dx.op.unary.f32(i32 21, float %211)  ; Exp(value)
  %213 = fmul fast float %208, %212
  %214 = fadd fast float %213, %207
  %215 = uitofp i32 %23 to float
  %216 = fmul fast float %215, -6.521250e+02
  %217 = call float @dx.op.unary.f32(i32 13, float %216)  ; Sin(value)
  %218 = fmul fast float %217, 0x40934BF580000000
  %219 = call float @dx.op.unary.f32(i32 22, float %218)  ; Frc(value)
  %220 = fadd fast float %120, %118
  %221 = fadd fast float %220, %119
  %222 = fmul fast float %221, -6.521250e+02
  %223 = call float @dx.op.unary.f32(i32 13, float %222)  ; Sin(value)
  %224 = fmul fast float %223, 0x40934BF580000000
  %225 = call float @dx.op.unary.f32(i32 22, float %224)  ; Frc(value)
  %226 = fadd fast float %225, %219
  %227 = fadd fast float %188, %189
  %228 = fadd fast float %227, %190
  %229 = fmul fast float %228, -6.521250e+02
  %230 = call float @dx.op.unary.f32(i32 13, float %229)  ; Sin(value)
  %231 = fmul fast float %230, 0x40934BF580000000
  %232 = call float @dx.op.unary.f32(i32 22, float %231)  ; Frc(value)
  %233 = fadd fast float %226, %232
  %234 = fadd fast float %159, %160
  %235 = fadd fast float %234, %161
  %236 = fmul fast float %163, -6.521250e+02
  %237 = fmul fast float %236, %235
  %238 = call float @dx.op.unary.f32(i32 13, float %237)  ; Sin(value)
  %239 = fmul fast float %238, 0x40934BF580000000
  %240 = call float @dx.op.unary.f32(i32 22, float %239)  ; Frc(value)
  %241 = fadd fast float %233, %240
  %242 = fmul fast float %241, -6.521250e+02
  %243 = call float @dx.op.unary.f32(i32 13, float %242)  ; Sin(value)
  %244 = fmul fast float %243, 0x40934BF580000000
  %245 = call float @dx.op.unary.f32(i32 22, float %244)  ; Frc(value)
  %246 = fcmp fast ogt float %214, %245
  br i1 %246, label %247, label %275

; <label>:247                                     ; preds = %203, %183
  %248 = call float @dx.op.dot3.f32(i32 55, float %164, float %165, float %166, float %188, float %189, float %190)  ; Dot3(ax,ay,az,bx,by,bz)
  %249 = fmul fast float %248, 2.000000e+00
  %250 = fmul fast float %249, %188
  %251 = fmul fast float %249, %189
  %252 = fmul fast float %249, %190
  %253 = fsub fast float %164, %250
  %254 = fsub fast float %165, %251
  %255 = fsub fast float %166, %252
  %256 = fmul fast float %188, 0x3EE4F8B580000000
  %257 = fmul fast float %189, 0x3EE4F8B580000000
  %258 = fmul fast float %190, 0x3EE4F8B580000000
  %259 = fadd fast float %256, %118
  %260 = fadd fast float %257, %119
  %261 = fadd fast float %258, %120
  %262 = insertelement <3 x float> undef, float %259, i32 0
  %263 = insertelement <3 x float> %262, float %260, i32 1
  %264 = insertelement <3 x float> %263, float %261, i32 2
  %265 = getelementptr inbounds %struct.Payload, %struct.Payload* %payload, i32 0, i32 2
  store <3 x float> %264, <3 x float>* %265, align 4, !tbaa !40
  %266 = call float @dx.op.dot3.f32(i32 55, float %253, float %254, float %255, float %253, float %254, float %255)  ; Dot3(ax,ay,az,bx,by,bz)
  %267 = call float @dx.op.unary.f32(i32 25, float %266)  ; Rsqrt(value)
  %268 = fmul fast float %253, %267
  %269 = fmul fast float %254, %267
  %270 = fmul fast float %255, %267
  %271 = insertelement <3 x float> undef, float %268, i32 0
  %272 = insertelement <3 x float> %271, float %269, i32 1
  %273 = insertelement <3 x float> %272, float %270, i32 2
  %274 = getelementptr inbounds %struct.Payload, %struct.Payload* %payload, i32 0, i32 3
  store <3 x float> %273, <3 x float>* %274, align 4, !tbaa !40
  br label %453

; <label>:275                                     ; preds = %203
  %276 = call float @dx.op.dot3.f32(i32 55, float %164, float %165, float %166, float %164, float %165, float %166)  ; Dot3(ax,ay,az,bx,by,bz)
  %277 = call float @dx.op.unary.f32(i32 25, float %276)  ; Rsqrt(value)
  %278 = fmul fast float %277, %164
  %279 = fmul fast float %277, %165
  %280 = fmul fast float %277, %166
  %281 = call float @dx.op.dot3.f32(i32 55, float %278, float %279, float %280, float %188, float %189, float %190)  ; Dot3(ax,ay,az,bx,by,bz)
  %282 = fmul fast float %184, %184
  %283 = fmul fast float %281, %281
  %284 = fsub fast float 1.000000e+00, %283
  %285 = fmul fast float %282, %284
  %286 = fsub fast float 1.000000e+00, %285
  %287 = fcmp fast oge float %286, 0.000000e+00
  %288 = fmul fast float %278, %184
  %289 = fmul fast float %279, %184
  %290 = fmul fast float %280, %184
  %291 = call float @dx.op.unary.f32(i32 24, float %286)  ; Sqrt(value)
  %292 = fmul fast float %281, %184
  %293 = fadd fast float %292, %291
  %294 = fmul fast float %293, %188
  %295 = fmul fast float %293, %189
  %296 = fmul fast float %293, %190
  %297 = fsub fast float %288, %294
  %298 = fsub fast float %289, %295
  %299 = fsub fast float %290, %296
  %300 = select i1 %287, float %297, float 0.000000e+00
  %301 = select i1 %287, float %298, float 0.000000e+00
  %302 = select i1 %287, float %299, float 0.000000e+00
  %303 = insertelement <3 x float> undef, float %300, i32 0
  %304 = insertelement <3 x float> %303, float %301, i32 1
  %305 = insertelement <3 x float> %304, float %302, i32 2
  %306 = getelementptr inbounds %struct.Payload, %struct.Payload* %payload, i32 0, i32 2
  store <3 x float> %123, <3 x float>* %306, align 4, !tbaa !40
  %307 = getelementptr inbounds %struct.Payload, %struct.Payload* %payload, i32 0, i32 3
  store <3 x float> %305, <3 x float>* %307, align 4, !tbaa !40
  br label %453

; <label>:308                                     ; preds = %180
  %309 = fcmp fast oeq float %19, 1.000000e+00
  %310 = call float @dx.op.dot3.f32(i32 55, float %118, float %119, float %120, float 0x4037240460000000, float 0x4005523720000000, float 0xC03BE7B540000000)  ; Dot3(ax,ay,az,bx,by,bz)
  %311 = call float @dx.op.unary.f32(i32 12, float %310)  ; Cos(value)
  %312 = fmul fast float %311, 0x40C81CD6E0000000
  %313 = call float @dx.op.unary.f32(i32 22, float %312)  ; Frc(value)
  %314 = fadd fast float %118, %120
  %315 = call float @dx.op.dot2.f32(i32 54, float %314, float %119, float 0x4039FAC720000000, float 0x40638EE980000000)  ; Dot2(ax,ay,bx,by)
  %316 = call float @dx.op.unary.f32(i32 13, float %315)  ; Sin(value)
  %317 = fmul fast float %316, 0x40E55DD180000000
  %318 = call float @dx.op.unary.f32(i32 22, float %317)  ; Frc(value)
  %319 = fmul fast float %119, %120
  %320 = call float @dx.op.dot2.f32(i32 54, float %319, float %118, float 0x4039FAC720000000, float 0x40638EE980000000)  ; Dot2(ax,ay,bx,by)
  %321 = call float @dx.op.unary.f32(i32 13, float %320)  ; Sin(value)
  %322 = fmul fast float %321, 0x40E55DD180000000
  %323 = call float @dx.op.unary.f32(i32 22, float %322)  ; Frc(value)
  %324 = fmul fast float %323, 2.000000e+00
  %325 = call float @dx.op.unary.f32(i32 6, float %324)  ; FAbs(value)
  %326 = fmul fast float %313, 0x400B4BC6A0000000
  %327 = fmul fast float %318, 0x400B4BC6A0000000
  %328 = fmul fast float %325, 0x3FFB4BC6A0000000
  %329 = uitofp i32 %23 to float
  %330 = fmul fast float %329, -6.521250e+02
  %331 = call float @dx.op.unary.f32(i32 13, float %330)  ; Sin(value)
  %332 = fmul fast float %331, 0x40934BF580000000
  %333 = call float @dx.op.unary.f32(i32 22, float %332)  ; Frc(value)
  %334 = fmul fast float %333, 0x4029666660000000
  %335 = fmul fast float %334, %164
  %336 = fmul fast float %334, %165
  %337 = fmul fast float %334, %166
  %338 = fsub fast float %326, %335
  %339 = fsub fast float %327, %336
  %340 = fsub fast float %328, %337
  %341 = call %dx.types.CBufRet.i32 @dx.op.cbufferLoadLegacy.i32(i32 59, %dx.types.Handle %5, i32 5)  ; CBufferLoadLegacy(handle,regIndex)
  %342 = extractvalue %dx.types.CBufRet.i32 %341, 0
  %343 = uitofp i32 %342 to float
  %344 = fmul fast float %343, -6.521250e+02
  %345 = call float @dx.op.unary.f32(i32 13, float %344)  ; Sin(value)
  %346 = fmul fast float %345, 0x40934BF580000000
  %347 = call float @dx.op.unary.f32(i32 22, float %346)  ; Frc(value)
  %348 = fmul fast float %347, 0x40147C5D60000000
  %349 = fmul fast float %348, %175
  %350 = fmul fast float %348, %176
  %351 = fmul fast float %348, %177
  %352 = fadd fast float %349, %338
  %353 = fadd fast float %350, %339
  %354 = fadd fast float %351, %340
  %355 = call float @dx.op.dot3.f32(i32 55, float %352, float %353, float %354, float 0x4037240460000000, float 0x4005523720000000, float 0xC03BE7B540000000)  ; Dot3(ax,ay,az,bx,by,bz)
  %356 = call float @dx.op.unary.f32(i32 12, float %355)  ; Cos(value)
  %357 = fmul fast float %356, 0x40C81CD6E0000000
  %358 = call float @dx.op.unary.f32(i32 22, float %357)  ; Frc(value)
  %359 = fadd fast float %352, %354
  %360 = call float @dx.op.dot2.f32(i32 54, float %359, float %353, float 0x4039FAC720000000, float 0x40638EE980000000)  ; Dot2(ax,ay,bx,by)
  %361 = call float @dx.op.unary.f32(i32 13, float %360)  ; Sin(value)
  %362 = fmul fast float %361, 0x40E55DD180000000
  %363 = call float @dx.op.unary.f32(i32 22, float %362)  ; Frc(value)
  %364 = fmul fast float %353, %354
  %365 = call float @dx.op.dot2.f32(i32 54, float %364, float %352, float 0x4039FAC720000000, float 0x40638EE980000000)  ; Dot2(ax,ay,bx,by)
  %366 = call float @dx.op.unary.f32(i32 13, float %365)  ; Sin(value)
  %367 = fmul fast float %366, 0x40E55DD180000000
  %368 = call float @dx.op.unary.f32(i32 22, float %367)  ; Frc(value)
  %369 = fmul fast float %368, 2.000000e+00
  %370 = call float @dx.op.unary.f32(i32 6, float %369)  ; FAbs(value)
  %371 = fmul fast float %358, 2.000000e+00
  %372 = fmul fast float %363, 2.000000e+00
  %373 = fadd fast float %371, -1.000000e+00
  %374 = fadd fast float %372, -1.000000e+00
  %375 = fadd fast float %370, -1.000000e+00
  %376 = fmul fast float %373, %373
  %377 = fmul fast float %374, %374
  %378 = fadd fast float %377, %376
  %379 = fmul fast float %375, %375
  %380 = fadd fast float %378, %379
  %381 = call float @dx.op.unary.f32(i32 24, float %380)  ; Sqrt(value)
  %382 = fcmp fast oeq float %381, 0.000000e+00
  %383 = fadd fast float %371, 0xBFEFFFFDE0000000
  %384 = fadd fast float %372, 0xBFEFFFFDE0000000
  %385 = fadd fast float %370, 0xBFEFFFFDE0000000
  %386 = select i1 %382, float %383, float %373
  %387 = select i1 %382, float %384, float %374
  %388 = select i1 %382, float %385, float %375
  %389 = call float @dx.op.dot3.f32(i32 55, float %386, float %387, float %388, float %386, float %387, float %388)  ; Dot3(ax,ay,az,bx,by,bz)
  %390 = call float @dx.op.unary.f32(i32 25, float %389)  ; Rsqrt(value)
  %391 = getelementptr inbounds %struct.Payload, %struct.Payload* %payload, i32 0, i32 2
  %392 = getelementptr inbounds %struct.Payload, %struct.Payload* %payload, i32 0, i32 3
  br i1 %309, label %393, label %428

; <label>:393                                     ; preds = %308
  %394 = call float @dx.op.binary.f32(i32 35, float %20, float 0.000000e+00)  ; FMax(a,b)
  %395 = call float @dx.op.binary.f32(i32 36, float %394, float 0x3FE99999A0000000)  ; FMin(a,b)
  %396 = fmul fast float %395, %390
  %397 = fmul fast float %396, %386
  %398 = fmul fast float %396, %387
  %399 = fmul fast float %396, %388
  %400 = call float @dx.op.dot3.f32(i32 55, float %164, float %165, float %166, float %175, float %176, float %177)  ; Dot3(ax,ay,az,bx,by,bz)
  %401 = fmul fast float %400, 2.000000e+00
  %402 = fmul fast float %401, %175
  %403 = fmul fast float %401, %176
  %404 = fmul fast float %401, %177
  %405 = fsub fast float %164, %402
  %406 = fsub fast float %165, %403
  %407 = fsub fast float %166, %404
  %408 = fmul fast float %175, 0x3EE4F8B580000000
  %409 = fmul fast float %176, 0x3EE4F8B580000000
  %410 = fmul fast float %177, 0x3EE4F8B580000000
  %411 = fadd fast float %408, %118
  %412 = fadd fast float %409, %119
  %413 = fadd fast float %410, %120
  %414 = insertelement <3 x float> undef, float %411, i32 0
  %415 = insertelement <3 x float> %414, float %412, i32 1
  %416 = insertelement <3 x float> %415, float %413, i32 2
  store <3 x float> %416, <3 x float>* %391, align 4, !tbaa !40
  %417 = call float @dx.op.dot3.f32(i32 55, float %405, float %406, float %407, float %405, float %406, float %407)  ; Dot3(ax,ay,az,bx,by,bz)
  %418 = call float @dx.op.unary.f32(i32 25, float %417)  ; Rsqrt(value)
  %419 = fmul fast float %405, %418
  %420 = fmul fast float %406, %418
  %421 = fmul fast float %407, %418
  %422 = fadd fast float %419, %397
  %423 = fadd fast float %420, %398
  %424 = fadd fast float %421, %399
  %425 = insertelement <3 x float> undef, float %422, i32 0
  %426 = insertelement <3 x float> %425, float %423, i32 1
  %427 = insertelement <3 x float> %426, float %424, i32 2
  store <3 x float> %427, <3 x float>* %392, align 4, !tbaa !40
  br label %453

; <label>:428                                     ; preds = %308
  %429 = fmul fast float %386, %390
  %430 = fmul fast float %387, %390
  %431 = fmul fast float %388, %390
  %432 = call float @dx.op.dot3.f32(i32 55, float %175, float %176, float %177, float %175, float %176, float %177)  ; Dot3(ax,ay,az,bx,by,bz)
  %433 = call float @dx.op.unary.f32(i32 25, float %432)  ; Rsqrt(value)
  %434 = fmul fast float %433, %175
  %435 = fmul fast float %433, %176
  %436 = fmul fast float %433, %177
  %437 = fadd fast float %434, %429
  %438 = fadd fast float %435, %430
  %439 = fadd fast float %436, %431
  %440 = fmul fast float %437, %437
  %441 = fmul fast float %438, %438
  %442 = fadd fast float %440, %441
  %443 = fmul fast float %439, %439
  %444 = fadd fast float %442, %443
  %445 = call float @dx.op.unary.f32(i32 24, float %444)  ; Sqrt(value)
  %446 = fcmp fast olt float %445, 0x3EB0C6F7A0000000
  %447 = select i1 %446, float %434, float %437
  %448 = select i1 %446, float %435, float %438
  %449 = select i1 %446, float %436, float %439
  %450 = insertelement <3 x float> undef, float %447, i32 0
  %451 = insertelement <3 x float> %450, float %448, i32 1
  %452 = insertelement <3 x float> %451, float %449, i32 2
  store <3 x float> %123, <3 x float>* %391, align 4, !tbaa !40
  store <3 x float> %452, <3 x float>* %392, align 4, !tbaa !40
  br label %453

; <label>:453                                     ; preds = %428, %393, %275, %247
  %454 = getelementptr inbounds %struct.Payload, %struct.Payload* %payload, i32 0, i32 0
  %455 = load <3 x float>, <3 x float>* %454, align 4, !tbaa !40
  %456 = extractelement <3 x float> %455, i32 0
  %457 = fmul fast float %456, %15
  %458 = extractelement <3 x float> %455, i32 1
  %459 = fmul fast float %458, %16
  %460 = extractelement <3 x float> %455, i32 2
  %461 = fmul fast float %460, %17
  %462 = insertelement <3 x float> undef, float %457, i32 0
  %463 = insertelement <3 x float> %462, float %459, i32 1
  %464 = insertelement <3 x float> %463, float %461, i32 2
  store <3 x float> %464, <3 x float>* %454, align 4, !tbaa !40
  br label %472

; <label>:465                                     ; preds = %45
  %466 = getelementptr inbounds %struct.Payload, %struct.Payload* %payload, i32 0, i32 0
  %467 = load <3 x float>, <3 x float>* %466, align 4, !tbaa !40
  %468 = extractelement <3 x float> %467, i32 0
  %469 = insertelement <3 x float> undef, float %468, i32 0
  %470 = insertelement <3 x float> %469, float 0.000000e+00, i32 1
  %471 = shufflevector <3 x float> %470, <3 x float> %467, <3 x i32> <i32 0, i32 1, i32 5>
  store <3 x float> %471, <3 x float>* %466, align 4, !tbaa !40
  br label %472

; <label>:472                                     ; preds = %465, %453, %31
  ret void
}

; Function Attrs: nounwind readonly
declare %dx.types.ResRet.f32 @dx.op.rawBufferLoad.f32(i32, %dx.types.Handle, i32, i32, i8, i32) #1

; Function Attrs: nounwind readonly
declare %dx.types.ResRet.i32 @dx.op.rawBufferLoad.i32(i32, %dx.types.Handle, i32, i32, i8, i32) #1

; Function Attrs: nounwind readnone
declare float @dx.op.unary.f32(i32, float) #2

; Function Attrs: nounwind readnone
declare float @dx.op.dot2.f32(i32, float, float, float, float) #2

; Function Attrs: nounwind readnone
declare float @dx.op.dot3.f32(i32, float, float, float, float, float, float) #2

; Function Attrs: nounwind readnone
declare float @dx.op.binary.f32(i32, float, float) #2

; Function Attrs: nounwind readnone
declare i32 @dx.op.dispatchRaysDimensions.i32(i32, i8) #2

; Function Attrs: nounwind readnone
declare i32 @dx.op.dispatchRaysIndex.i32(i32, i8) #2

; Function Attrs: nounwind
declare void @dx.op.traceRay.struct.Payload(i32, %dx.types.Handle, i32, i32, i32, i32, i32, float, float, float, float, float, float, float, float, %struct.Payload*) #0

; Function Attrs: nounwind readonly
declare %dx.types.ResRet.f32 @dx.op.textureLoad.f32(i32, %dx.types.Handle, i32, i32, i32, i32, i32, i32, i32) #1

; Function Attrs: nounwind
declare void @dx.op.textureStore.f32(i32, %dx.types.Handle, i32, i32, i32, float, float, float, float, i8) #0

; Function Attrs: nounwind readnone
declare float @dx.op.worldRayOrigin.f32(i32, i8) #2

; Function Attrs: nounwind readnone
declare float @dx.op.worldRayDirection.f32(i32, i8) #2

; Function Attrs: nounwind readnone
declare i32 @dx.op.primitiveIndex.i32(i32) #2

; Function Attrs: nounwind readnone
declare i32 @dx.op.instanceID.i32(i32) #2

; Function Attrs: nounwind readonly
declare float @dx.op.rayTCurrent.f32(i32) #1

; Function Attrs: nounwind readonly
declare %dx.types.CBufRet.i32 @dx.op.cbufferLoadLegacy.i32(i32, %dx.types.Handle, i32) #1

; Function Attrs: nounwind readonly
declare %dx.types.CBufRet.f32 @dx.op.cbufferLoadLegacy.f32(i32, %dx.types.Handle, i32) #1

; Function Attrs: nounwind readnone
declare float @dx.op.objectToWorld.f32(i32, i32, i8) #2

; Function Attrs: nounwind readnone
declare float @dx.op.tertiary.f32(i32, float, float, float) #2

; Function Attrs: nounwind readonly
declare %dx.types.Handle @dx.op.createHandleForLib.FrameConstantBuffer(i32, %FrameConstantBuffer) #1

; Function Attrs: nounwind readonly
declare %dx.types.Handle @"dx.op.createHandleForLib.class.StructuredBuffer<VertexRaw>"(i32, %"class.StructuredBuffer<VertexRaw>") #1

; Function Attrs: nounwind readonly
declare %dx.types.Handle @dx.op.createHandleForLib.struct.ByteAddressBuffer(i32, %struct.ByteAddressBuffer) #1

; Function Attrs: nounwind readonly
declare %dx.types.Handle @dx.op.createHandleForLib.struct.RaytracingAccelerationStructure(i32, %struct.RaytracingAccelerationStructure) #1

; Function Attrs: nounwind readonly
declare %dx.types.Handle @"dx.op.createHandleForLib.class.RWTexture2D<vector<float, 4> >"(i32, %"class.RWTexture2D<vector<float, 4> >") #1

; Function Attrs: nounwind readonly
declare %dx.types.Handle @"dx.op.createHandleForLib.class.StructuredBuffer<InstInfo>"(i32, %"class.StructuredBuffer<InstInfo>") #1

attributes #0 = { nounwind }
attributes #1 = { nounwind readonly }
attributes #2 = { nounwind readnone }

!llvm.ident = !{!0}
!dx.version = !{!1}
!dx.valver = !{!2}
!dx.shaderModel = !{!3}
!dx.resources = !{!4}
!dx.typeAnnotations = !{!19}
!dx.entryPoints = !{!27, !29, !32, !34}

!0 = !{!"dxcoob 1.8.2407.7 (416fab6b5)"}
!1 = !{i32 1, i32 3}
!2 = !{i32 1, i32 8}
!3 = !{!"lib", i32 6, i32 3}
!4 = !{!5, !13, !17, null}
!5 = !{!6, !8, !10, !12}
!6 = !{i32 0, %struct.RaytracingAccelerationStructure* @"\01?scene@@3URaytracingAccelerationStructure@@A", !"scene", i32 0, i32 0, i32 1, i32 16, i32 0, !7}
!7 = !{i32 0, i32 4}
!8 = !{i32 1, %"class.StructuredBuffer<InstInfo>"* @"\01?instsInfo@@3V?$StructuredBuffer@UInstInfo@@@@A", !"instsInfo", i32 0, i32 1, i32 1, i32 12, i32 0, !9}
!9 = !{i32 1, i32 64}
!10 = !{i32 2, %"class.StructuredBuffer<VertexRaw>"* @"\01?sceneVertices@@3V?$StructuredBuffer@UVertexRaw@@@@A", !"sceneVertices", i32 0, i32 2, i32 1, i32 12, i32 0, !11}
!11 = !{i32 1, i32 48}
!12 = !{i32 3, %struct.ByteAddressBuffer* @"\01?sceneIndices@@3UByteAddressBuffer@@A", !"sceneIndices", i32 0, i32 3, i32 1, i32 11, i32 0, null}
!13 = !{!14, !16}
!14 = !{i32 0, %"class.RWTexture2D<vector<float, 4> >"* @"\01?uavRTPresent@@3V?$RWTexture2D@V?$vector@M$03@@@@A", !"uavRTPresent", i32 0, i32 0, i32 1, i32 2, i1 false, i1 false, i1 false, !15}
!15 = !{i32 0, i32 9}
!16 = !{i32 1, %"class.RWTexture2D<vector<float, 4> >"* @"\01?uavRTRadiance@@3V?$RWTexture2D@V?$vector@M$03@@@@A", !"uavRTRadiance", i32 0, i32 1, i32 1, i32 2, i1 false, i1 false, i1 false, !15}
!17 = !{!18}
!18 = !{i32 0, %FrameConstantBuffer* @FrameConstantBuffer, !"FrameConstantBuffer", i32 0, i32 0, i32 1, i32 96, null}
!19 = !{i32 1, void ()* @"\01?RayGeneration@@YAXXZ", !20, void (%struct.Payload*)* @"\01?Miss@@YAXUPayload@@@Z", !23, void (%struct.Payload*, %struct.BuiltInTriangleIntersectionAttributes*)* @"\01?ClosestHit@@YAXUPayload@@UBuiltInTriangleIntersectionAttributes@@@Z", !25}
!20 = !{!21}
!21 = !{i32 1, !22, !22}
!22 = !{}
!23 = !{!21, !24}
!24 = !{i32 2, !22, !22}
!25 = !{!21, !24, !26}
!26 = !{i32 0, !22, !22}
!27 = !{null, !"", null, !4, !28}
!28 = !{i32 0, i64 8212}
!29 = !{void (%struct.Payload*, %struct.BuiltInTriangleIntersectionAttributes*)* @"\01?ClosestHit@@YAXUPayload@@UBuiltInTriangleIntersectionAttributes@@@Z", !"\01?ClosestHit@@YAXUPayload@@UBuiltInTriangleIntersectionAttributes@@@Z", null, null, !30}
!30 = !{i32 8, i32 10, i32 6, i32 60, i32 7, i32 8, i32 5, !31}
!31 = !{i32 0}
!32 = !{void (%struct.Payload*)* @"\01?Miss@@YAXUPayload@@@Z", !"\01?Miss@@YAXUPayload@@@Z", null, null, !33}
!33 = !{i32 8, i32 11, i32 6, i32 60, i32 5, !31}
!34 = !{void ()* @"\01?RayGeneration@@YAXXZ", !"\01?RayGeneration@@YAXXZ", null, null, !35}
!35 = !{i32 8, i32 7, i32 5, !31}
!36 = !{!37, !37, i64 0}
!37 = !{!"bool", !38, i64 0}
!38 = !{!"omnipotent char", !39, i64 0}
!39 = !{!"Simple C/C++ TBAA"}
!40 = !{!38, !38, i64 0}
!41 = !{!42, !42, i64 0}
!42 = !{!"int", !38, i64 0}
!43 = !{i32 0, i32 2}
!44 = !{!45, !47, !48, !49}
!45 = distinct !{!45, !46, !"\01?ParseVertex@@YAXIAIAV?$vector@M$02@@0AIAV?$vector@M$03@@AIAV?$vector@M$01@@@Z: %pos"}
!46 = distinct !{!46, !"\01?ParseVertex@@YAXIAIAV?$vector@M$02@@0AIAV?$vector@M$03@@AIAV?$vector@M$01@@@Z"}
!47 = distinct !{!47, !46, !"\01?ParseVertex@@YAXIAIAV?$vector@M$02@@0AIAV?$vector@M$03@@AIAV?$vector@M$01@@@Z: %normal"}
!48 = distinct !{!48, !46, !"\01?ParseVertex@@YAXIAIAV?$vector@M$02@@0AIAV?$vector@M$03@@AIAV?$vector@M$01@@@Z: %tangent"}
!49 = distinct !{!49, !46, !"\01?ParseVertex@@YAXIAIAV?$vector@M$02@@0AIAV?$vector@M$03@@AIAV?$vector@M$01@@@Z: %uv"}

#endif

const unsigned char CustomRTShader[] = {
  
};
